...
import { initializeDatabase, dbGetAll, dbGetById, dbRun } from './database/database.js'

const app = express()
app.use(express.json())

app.get('/wizard', async(req, res) => {
    const wizards = await dbGetAll("SELECT * FROM roxfort");
    res.status(200).json(wizards)
})

app.get("/wizard/:id", async (req, res) => {
    const id = req.params.id;
    const wizard = await dbGetById("SELECT * FROM roxfort WHERE id = ?;", [id])
    if (!wizard) {
...
app.put("/wizard/:id", async (req, res) => {
    const id = req.params.id;
    const wizard = dbGetById("SELECT * FROM roxfort WHERE id = ?;", [id]);
    if (!wizard) {
